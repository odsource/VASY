...
void printRouteTable(int i) {
    Serial.print("\r\n");
    Serial.print("ROUTING TABLE ENTRY DESTINATION: \r\n");
    Serial.print(routingTable[i].dest, HEX);
    Serial.print("\r\n");
    Serial.print("ROUTING TABLE ENTRY NEXT HOP: \r\n");
    Serial.print(routingTable[i].next, HEX);
    Serial.print("\r\n");
    Serial.print("ROUTING TABLE ENTRY PATH COST: \r\n");
    Serial.print(routingTable[i].cost);
    Serial.print("\r\n");
    Serial.print("\r\n");
}
void sendTx() {
    ...
    if (xbee.getResponse().isAvailable()) {
        ...
        if (xbee.getResponse().getApiId() == RX_16_RESPONSE || xbee.getResponse().getApiId() == RX_64_RESPONSE) {
            ...
            if (xbee.getResponse().getApiId() == RX_64_RESPONSE) {
                ...
                Serial.print("\r\n");
                Serial.print("RECEIVED SRC: \r\n");
                Serial.print(deciphered.src, HEX);
                Serial.print("\r\n");
                Serial.print("RECEIVED DEST: \r\n");
                Serial.print(deciphered.dest, HEX);
                Serial.print("\r\n");
                Serial.print("RECEIVED ID: \r\n");
                Serial.print(deciphered.id);
                Serial.print("\r\n");
                ...
                if(pl[2] == LOCAL_SRC) {
                    ...
                    for(int i = 0; i < ARRAY_SIZE; i++) {
                        ...
                        if (pl[0] == 2) {
                            ...
                            if(sent[i].flag == 15) {
                                Serial.print("NEW FLOODING BROADCAST MESSAGE RECEIVED\r\n");
                                flag = 0;
                                break;
                                ...
                                flag = 1;
                                Serial.print("OLD FLOODING MESSAGE RECEIVED\r\n");
                                break;
                            }
                        }
                    }
...
/*****************************************************************************************************************************************************/   
                ...
                // Type 0..3 -> 0 = Normal, 1 = ACK, 2 = RREQ, 3 = RREP
                    ...
                    for(int i = 0; i < ARRAY_SIZE; i++) {
                        ...
                        // Normal message
                        if(sent[i].flag == 15) {
                            Serial.print("FLOODING RECEIVED\r\n");
                            flag = 0;
                            break;
                        } else if((sent[i].flag == deciphered.flag) &&
                            (sent[i].src == deciphered.src) &&
                            (sent[i].dest == deciphered.dest) &&
                            (sent[i].id == deciphered.id)){
                            Serial.print("MESSAGE ALREADY RECEIVED\r\n");
                            // flag == 1 bedeutet, dass das empfangene Paket bereits weitergeleitet wurde
                            flag = 1;
                            break;
                        }
                    ...
                    if (flag == 0) {
                        ...
                        if (pl[0] == 2) {
                            ...
                            for (int i = 0; i < ARRAY_SIZE; i++) {
                                ...
                                if ((routeDiscoveryTable[i].src == 0) &&
                                    (routeDiscoveryTable[i].dest == 0)) {
                                    Serial.print("NO ENTRY IN ROUTEDISCOVERYTABLE\r\n");
                                    routeDiscoveryTable[i].src = deciphered.src;
                                    routeDiscoveryTable[i].dest = deciphered.dest;
                                    routeDiscoveryTable[i].prev = rx16.getRemoteAddress16();
                                    routeDiscoveryTable[i].forward_cost = deciphered.path_cost + 1;
                                    ...
                                    (routeDiscoveryTable[i].dest == deciphered.dest))) {
                                    Serial.print("ENTRY IN ROUTEDISCOVERYTABLE\r\n");
                                    if (routeDiscoveryTable[i].forward_cost >= deciphered.path_cost+1) {
                                        routeDiscoveryTable[i].forward_cost = deciphered.path_cost + 1;
                                        routeDiscoveryTable[i].prev = rx16.getRemoteAddress16();
                                        if (routed == 1) {
